...
import { Brain, Terminal, GitBranch, Bot, CreditCard, Send, Server, Key, ChevronRight, RefreshCw, ShieldCheck, Database, AlertTriangle } from 'lucide-react';
...
export default function App() {
  ...
  const [settings, setSettings] = useState<AppSettings>(() => {
     try {
         ...
         return { godotWsUrl: '', githubToken: '', githubRepo: '', gameEndpoints: [] };
     }
  });
  ...
  const handleSaveSettings = (newSettings: AppSettings) => {
      ...
      if (newSettings.apiKey || newSettings.superKey) {
          setApiKeyReady(true);
      }
  };
  ...
  // CHECK KEY ON LOAD
  useEffect(() => {
      if (settings.apiKey || settings.superKey || process.env.API_KEY) {
          setApiKeyReady(true);
      }
  }, []);

  const handleSaveKey = () => {
      // 1. Try JSON Injection (Triple Key)
      try {
          if (inputKey.trim().startsWith('{')) {
              ...
              if (keys.gemini) {
                  localStorage.setItem('feac_api_key', keys.gemini);
                  newSettings.apiKey = keys.gemini;
                  injectedCount++;
              }
              if (keys.flowith) {
                  localStorage.setItem('feac_flowith_key', keys.flowith);
                  newSettings.flowithApiKey = keys.flowith;
                  injectedCount++;
              }
              if (keys.super) {
                  localStorage.setItem('feac_super_key', keys.super);
                  newSettings.superKey = keys.super;
                  injectedCount++;
              }
              
              setSettings(newSettings);
              setApiKeyReady(true);
              alert(`⚡ TRIPLE INJECTION SUCCESSFUL. ${injectedCount} KEYS UPDATED.`);
              return;
          }
      } catch (e) {
          // Not JSON, continue to single key check
      }

      // 2. Single Key Check
      if (inputKey.startsWith('FEAC-SVR-')) {
          // Sovereign Bypass
          localStorage.setItem('feac_super_key', inputKey);
          setSettings(prev => ({...prev, superKey: inputKey}));
          setApiKeyReady(true);
          alert("Sovereign Access Granted. Configure AI Model Keys in Settings.");
      } else if (inputKey.length > 10) {
          localStorage.setItem('feac_api_key', inputKey);
          setSettings(prev => ({...prev, apiKey: inputKey}));
          setApiKeyReady(true);
      } else {
          alert("Invalid Key. Paste Single Key or JSON: {\"gemini\":\"...\",\"flowith\":\"...\",\"super\":\"...\"}");
      }
  };
      ...
      return (
          <div className="h-screen w-screen bg-[#0b141a] flex flex-col items-center justify-center text-[#e9edef] p-6 text-center font-sans">
              ...
              <p className="text-gray-400 mb-8 max-w-xs text-sm">
                  Inject Keys to activate FEAC Sovereign Core.
                  <br/>Supported: Gemini, Flowith, FEAC-SVR.
              </p>
              ...
              <div className="w-full max-w-sm space-y-4">
                  <div className="bg-[#1f2c34] p-2 rounded-lg flex items-center border border-gray-700">
                      ...
                      <input 
                        ...
                        placeholder='Key or {"gemini":"...", ...}'
                        className="bg-transparent border-none outline-none text-white text-sm p-2 flex-1 w-full font-mono"
                        type="password"
                      />
                  </div>
                  <button 
                    onClick={handleSaveKey}
                    className="w-full bg-green-600 hover:bg-green-500 text-white py-3 rounded-lg font-bold shadow-lg flex items-center justify-center gap-2"
                  >
             ...
             <div className="bg-[#0b141a] rounded-lg shadow-2xl max-w-sm w-full border border-green-500/30 overflow-hidden">
                <div className="bg-green-900/20 p-4 flex items-center gap-3 border-b border-white/5">
                    <div className="bg-green-500/20 p-2 rounded-full">
                        <RefreshCw size={20} className="text-green-400 animate-spin"/>
                    </div>
                    <div>
                        <h3 className="text-sm font-bold text-green-400 tracking-widest uppercase">NEURAL CORE UPDATE</h3>
                        <p className="text-xs text-green-600 font-mono">v{feacVersion}</p>
                    </div>
                </div>
                <div className="p-6">
                    <p className="text-slate-400 mb-6 text-sm">Neural Core Update available. System optimization complete.</p>
                    <div className="space-y-1 mb-6">
                        <div className="flex justify-between text-[10px] text-green-500 font-bold">
                    ...
                    <div className="bg-[#202c33] p-4 rounded text-center mb-4">
                         <h4 className="text-white font-bold">Updated! Reloading...</h4>
                    </div>
                    <div className="flex justify-end">
                       <button 
                         onClick={() => window.location.reload()}
                         className="w-full bg-green-600 hover:bg-green-500 text-white font-bold uppercase tracking-widest text-xs py-3 rounded transition-colors shadow-lg"
                       >
                         OK
                       </button>
                    </div>
           ...
           <div className="flex flex-col h-full bg-[#0b141a]">
               ...
               {!settings.apiKey && (
                   <div className="bg-red-900/50 p-2 text-center text-xs text-red-300 font-bold border-b border-red-500 flex items-center justify-center gap-2 cursor-pointer" onClick={() => setShowSettings(true)}>
                       <AlertTriangle size={14}/> API KEY MISSING OR REVOKED. TAP TO CONFIGURE.
                   </div>
               )}
