...
import { X, Save, Gamepad2, Github, Key, Network, Plus, Trash2 } from 'lucide-react';
      ...
      <div className="bg-[#202c33] w-full max-w-lg rounded-lg shadow-2xl border border-slate-700 flex flex-col max-h-[90vh]">
        <div className="p-4 border-b border-slate-700 flex justify-between items-center bg-[#111b21] rounded-t-lg">
          <h2 className="text-white font-bold flex items-center gap-2">
            <Key size={18} className="text-green-400" /> System Integrations
          </h2>
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          ...
          <div className="space-y-3">
             <div className="flex items-center gap-2 text-blue-400 font-bold border-b border-blue-500/20 pb-1">
                <Gamepad2 size={18} /> NeoEngine / Godot Bridge
             </div>
             <div>
                <label className="block text-xs text-gray-400 mb-1">Primary WebSocket URL</label>
                <input 
                  className="w-full bg-[#111b21] p-3 rounded text-sm text-white border border-slate-700 focus:border-blue-500 outline-none font-mono"
                  placeholder="ws://127.0.0.1:8090"
                  value={form.godotWsUrl}
                  onChange={e => setForm({...form, godotWsUrl: e.target.value})}
                />
             </div>
          </div>
             ...
             <div className="space-y-2">
                 {(form.gameEndpoints || []).map(port => (
                     <div key={port.id} className="flex gap-2 items-center bg-[#111b21] p-2 rounded border border-slate-700">
                         <input 
                            className="w-20 bg-black/30 p-1 rounded text-xs text-green-400 font-mono text-center" 
                            value={port.port}
                            onChange={e => updatePort(port.id, 'port', e.target.value)}
                            placeholder="8080"
                         />
                         <input 
                            className="flex-1 bg-transparent p-1 text-sm text-white outline-none" 
                            value={port.name}
                            onChange={e => updatePort(port.id, 'name', e.target.value)}
                            placeholder="Service Name"
                         />
                         <select 
                            className="bg-black/30 text-xs text-slate-400 p-1 rounded"
                            value={port.type}
                            onChange={e => updatePort(port.id, 'type', e.target.value as any)}
                         >
                             <option value="telemetry">Telemetry</option>
                             <option value="control">Control</option>
                             <option value="iap_verification">IAP</option>
                         </select>
                         <button onClick={() => removePort(port.id)} className="text-red-500 hover:text-red-400"><Trash2 size={14}/></button>
                     </div>
                 ))}
             </div>
             <p className="text-[10px] text-gray-500">FEAC listens on these ports to analyze user behavior & IAP events.</p>
          </div>
        </div>
        ...
        <div className="p-4 bg-[#111b21] border-t border-slate-700 rounded-b-lg">
           <button 
             ...
             className="w-full bg-green-600 hover:bg-green-500 text-white py-3 rounded font-bold flex items-center justify-center gap-2 transition-colors shadow-lg"
           >
